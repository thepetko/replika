...
export function buildSchedule(units, options = {}) {
  ...
  const activeLockedPlans = Object.entries(lockedPlans).filter(([date]) => date >= start && date <= deadline && !daysOff.includes(date));
  const activeLockedDates = new Set(activeLockedPlans.map(([date]) => date));
  const activeLockedIds = new Set(activeLockedPlans.flatMap(([, ids]) => ids));
  const activeLockedUnits = activeLockedPlans.flatMap(([, ids]) => ids.map(id => byId.get(id)).filter(unit => unit && !unit.completedAt));
  const remaining = chronological(units).filter(unit => !unit.completedAt && !activeLockedIds.has(unit.id));
  const protectedDates = activeDates.filter(date => date > newMaterialEnd);
  let learningDates = activeDates.filter(date => date >= start && date <= newMaterialEnd && !activeLockedDates.has(date));
  const remainingWeight = remaining.reduce((sum, unit) => sum + unitWeight(unit, weakPenalty), 0);
  for (const date of protectedDates) if (!activeLockedDates.has(date) && (!learningDates.length || remainingWeight / learningDates.length > overload || start > newMaterialEnd)) learningDates.push(date);
  ...
  for (const [date, ids] of activeLockedPlans) if (schedule[date]) schedule[date] = ids.map(id => byId.get(id)).filter(unit => unit && !unit.completedAt);
  ...
  const totalMinutes = [...remaining, ...activeLockedUnits].reduce((sum, unit) => sum + Number(unit.minutes || 0), 0);
  const lockedDaysWithWork = activeLockedPlans.filter(([, ids]) => ids.some(id => { const unit = byId.get(id); return unit && !unit.completedAt; })).length;
  const usable = learningDates.length + lockedDaysWithWork;
  return { schedule, reviews, dates: activeDates, allDates, totalMinutes, totalWeight: remainingWeight, requiredDailyMinutes: usable ? Math.ceil(totalMinutes / usable) : totalMinutes, impossible: Boolean((remaining.length || activeLockedUnits.length) && !usable), usedReviewDays: [...learningDates, ...activeLockedDates].some(date => date > newMaterialEnd) };
}
